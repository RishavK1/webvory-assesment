...
so migrations always target the same schema and the same connection string
the app uses — there is no second copy of that configuration to drift.
"""
...
def run_migrations_online() -> None:
    ...
    with connectable.connect() as connection:
        context.configure(
            ...
            # the table instead, so the same migration runs on both backends.
            render_as_batch=settings.is_sqlite,
        )
        with context.begin_transaction():
            context.run_migrations()
